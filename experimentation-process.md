...
<img src="/uploads/onboarding/onboarding-process-service-access.png" width="1024">
...
Next Figure displays the Patras5G vertical onboarding process ( Our process is based on the whitepaper https://doi.org/10.5281/zenodo.3695716  ). 
The process consists of several phases which are discussed in detail in subsequent sections.
i) The co-design period which involves stakeholders such as vertical customers, Patras5G facility providers and CFS/RFS developers to understand the vertical needs and how to enable them.
ii) The iterative co-development period which involves the vertical customer and the Patras5G facility provider to jointly develop the final service to be ordered including the development of any VNFs, test scripts and monitoring services. Testing as a Service (TaaS) and Monitoring as a Service (MaaS) can be integrated during the preparation of the VNFs and development of service templates. This is subject to availability of TaaS and/or MaaS in the Patras5G facility site.
...
During this initial period the involved stakeholders such as vertical customer, Patras5G facility providers, CFS/RFS developers need to understand the vertical customer needs from the 5G facility in terms of:
...
*  vertical requirements from the 5G system and the capabilities of the Patras5G facility.
...
#### DESIGN THE SERVICE SPECIFICATION (SERVICE BLUEPRINT)

During this process, the Vertical and the Patras5G facility can define the attributes of the Service Specification/Service Blueprint. This step might need some research on defining the proper parameter values in which the stakeholders need to be pragmatic on how these requirements are “translated” towards the SO and NFVO. In later stages, the Service Specification will be used to automatically trigger the complete orchestration of the network slice instance. Proper definition of the Service Specification requires some work initially but this will benefit the vertical customer to consequently reproduce the service instantiation via the service catalogues at later stages with potential minor updates to the Service Specification.

#### TEST CASE DESIGN (If available)

The Service Specification will have the option to include testing as a service (TaaS) a service developed in European Project 5G-VINNI. For example, by including relevant test scripts for TaaS. The test scripts will represent specific Test Cases (TCs) that are targeted at stressing specific aspects or KPIs of the Service. It is important to understand that there are two types of TCs: 1) TCs for validating fundamental network KPIs like throughput, delay, and 2) TCs for validating the service of the vertical customer that might include the vertical’s application. While the former can be offered as part of the service design with minimal configuration, the latter might require significant work.
...
At this point the Service Specification has been co-created with the vertical. The next step is to test and validate its proper operation via the SO. This involves pre-launching/pre-ordering the Service Specification via the Patras5G portal to check that the Service Specification can be correctly orchestrated via the SO. In this step, the Patras5G facility tries to instantiate, test and validate the defined Service Specification. This is also related to developments in Openslice itself as well as development in the SO part. During this step, the Service Specification attributes might be redefined based on the interaction with the Patras5G facility.

#### LAUNCH SERVICE SPEC - ALPHA/BETA VERSIONS

Since the Service Specification is instantiated and validated successfully, the  facility can start launching different versions of the Service Specification towards the SO and NFVO while involving TaaS and MaaS, checking and assessing their interoperability.

#### PRE-VALIDATION

In this step, a pre-validation period can start and required adjustment can be made to the service order. The vertical customer and Patras5G facility can check that the KPIs can be extracted properly. Since the orchestration of Service Specification parts have been verified, the vertical customer and the Patras5G facility can schedule and perform a pre-validation of the service and validate the proper deployment of the Network Service. At this stage it can be verified that the developed CFS can extract the proper KPIs. This process might also involve TaaS. By invoking TaaS during service deployment, the vertical customer and Patras5G facility will validate that the Service Specification and TaaS/MaaS work as expected.
